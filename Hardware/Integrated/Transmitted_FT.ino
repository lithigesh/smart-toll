...
void setup() {
  ...
  if (!SD.begin(SD_CS)) {
    Serial.println("[WARN] SD Card FAILED; geofence disabled");
  } else {
    File f = SD.open("/geofence.csv");
    if (!f) {
      Serial.println("[WARN] geofence.csv missing; geofence disabled");
    } else {
...
void processGPS() {
  ...
  if (!gpsFreshAndValid()) return;
  if (fenceCount < 2) return; // cannot compute polyline distance
    ...
    for (int i = 0; i < fenceCount; i++) {
      if (fenceTxEnable[i]) {
        ...
        if (poleDist <= TOLL_POLE_RADIUS_METERS && !txAlreadySent) {
          sendZigBeePayload();
          totalDistanceMeters = 0.0;      // reset ONLY after TX
          txAlreadySent = true;
          break;
        }
      }
...
void sendZigBeePayload() {
  ...
  // Prefer GPS-derived UTC timestamp (more accurate than millis).
  // Format matches backend normalizer: "YYYY:MM:DD HH:MM:SS"
  ...
  if (gps.date.isValid() && gps.time.isValid()) {
    ...
    // Fallback: still provide a valid timestamp so API validation passes.
    snprintf(timestampStr, sizeof(timestampStr), "1970:01:01 00:00:00");
  }

  // Convert meters -> kilometers for backend compatibility
  double distanceKm = totalDistanceMeters / 1000.0;

  // Build v2 payload: base + seq + CRC
  char base[100];
  snprintf(
    base,
    sizeof(base),
  ...
  uint16_t crc = crc16_ccitt((const uint8_t*)base, strlen(base));

  char payload[120];
  snprintf(payload, sizeof(payload), "<%s|%04X>", base, crc);

  for (int attempt = 1; attempt <= TX_MAX_RETRIES; attempt++) {
    ZBTxRequest tx(destAddr, (uint8_t*)payload, strlen(payload));
    xbee.send(tx);

    // Retries are still useful on noisy links (especially when broadcasting, where delivery acks are not reliable).
    delay(TX_RETRY_DELAY_MS);
  }

  Serial.print("TX @ Toll Pole → ");
  Serial.println(payload);
  Serial.println("[INFO] TX attempted (broadcast/no-ack); receiver validates CRC");
