...
char rxBuffer[220];   // payload buffer (room for extra tokens like seq)

/* ================== FAULT TOLERANCE CONFIG ================== */
static const uint8_t  API_POST_RETRIES = 3;
static const uint32_t API_POST_TIMEOUT_MS = 6000;
static const uint32_t DEDUP_WINDOW_MS = 6000;
...
// Simple recent-hash dedup cache
static uint32_t recentHashes[10];
static uint32_t recentHashMs[10];
static uint8_t recentHashIdx = 0;
...
static bool seenRecently(uint32_t hash, uint32_t nowMs) {
  for (uint8_t i = 0; i < 10; i++) {
    if (recentHashes[i] == hash && (nowMs - recentHashMs[i]) < DEDUP_WINDOW_MS) {
      return true;
    }
  }

  recentHashes[recentHashIdx] = hash;
  recentHashMs[recentHashIdx] = nowMs;
  recentHashIdx = (recentHashIdx + 1) % 10;
  return false;
}
...
String getIsoTimestampUtc() {
  ...
  if (!getLocalTime(&timeInfo, 200)) {
    // If NTP hasn't synced, return an invalid timestamp so backend falls back to server time.
...
bool parseIncomingPayload(
  ...
  if (raw[0] == '{') {
    ...
    if (!err) {
      ...
      if (doc["seq"].is<uint32_t>()) sequence = doc["seq"].as<uint32_t>();
  ...
  // Example: <ESP32_...|13.082680|80.270700|124.50|34|OK>
  {
    ...
    if (s.indexOf('|') >= 0) {
      ...
      if (p1 > 0 && p2 > p1 && p3 > p2) {
        ...
        // Optional: sequence id in the last token (Transmitter_FT adds it)
        int lastSep = s.lastIndexOf('|');
        if (lastSep > 0 && lastSep < (int)s.length() - 1) {
          String lastTok = s.substring(lastSep + 1);
          lastTok.trim();
          bool numeric = true;
          for (int i = 0; i < (int)lastTok.length(); i++) {
            if (lastTok[i] < '0' || lastTok[i] > '9') { numeric = false; break; }
          }
          if (numeric && lastTok.length() > 0) {
            sequence = (uint32_t)lastTok.toInt();
          }
        }
...
bool postTollTransaction(
  ...
  HTTPClient http;
  http.setTimeout(API_POST_TIMEOUT_MS);
...
/* ================== TASKS ================== */
void xbeeRxTask(void *parameter) {
  while (true) {
    xbee.readPacket(200);
    ...
    if (xbee.getResponse().isAvailable()) {
      if (xbee.getResponse().getApiId() == ZB_RX_RESPONSE) {
        ...
        if (len > 0 && len < (int)sizeof(rxBuffer)) {
          ...
          strncpy(pkt.raw, rxBuffer, sizeof(pkt.raw) - 1);

          if (rxQueue) {
            // If queue is full, drop the packet rather than blocking RX.
...
void apiPostTask(void *parameter) {
  RxPacket pkt;
  while (true) {
    if (!rxQueue || xQueueReceive(rxQueue, &pkt, portMAX_DELAY) != pdPASS) {
      continue;
    }

    const uint32_t nowMs = millis();
    uint32_t hash = fnv1a32(pkt.raw);
    if (seenRecently(hash, nowMs)) {
      Serial.println("Duplicate RX (dedup) - skipping");
      continue;
    }
    ...
    // Rate limit (still useful even with dedup)
    if (nowMs - lastPostMs < MIN_POST_INTERVAL_MS) {
      showOnOLED("RX (rate-limit)", String(pkt.raw));
      continue;
    }
    ...
    bool parsedOk = parseIncomingPayload(pkt.raw, deviceId, startLat, startLon, totalDistanceKm, timestamp, seq);
    if (!parsedOk) {
      Serial.println("Payload parse failed");
      showOnOLED("RX Error", "Parse failed", "Bad payload");
      continue;
    }

    // Validate coordinates/distance to avoid spamming API with junk
    if (startLat < -90 || startLat > 90 || startLon < -180 || startLon > 180 || totalDistanceKm <= 0) {
      Serial.println("Invalid data (range check) - skipping");
      showOnOLED("RX Error", "Invalid range");
      continue;
    }
    ...
    int httpCode = 0;
    bool posted = false;
    for (uint8_t attempt = 1; attempt <= API_POST_RETRIES; attempt++) {
      posted = postTollTransaction(deviceId, startLat, startLon, totalDistanceKm, timestamp, apiResp, httpCode);

      // httpCode <= 0 => transport failure
      if (!posted || httpCode <= 0) {
        Serial.println("POST transport fail - retrying");
      } else if (httpCode >= 500) {
        Serial.println("POST server error - retrying");
      } else {
        // 2xx/4xx: don't retry
        break;
      }

      vTaskDelay((attempt * 250) / portTICK_PERIOD_MS);
    }
    ...
    if (msg.length() == 0) msg = "(no msg)";

    if (posted && httpCode > 0) {
      showOnOLED("API", String("HTTP ") + httpCode, msg);
    } else {
      showOnOLED("API FAIL", String("HTTP ") + httpCode);
    }

    // Log sequence if present
    if (seq > 0) {
      Serial.print("Seq: ");
      Serial.println(seq);
    }
...
void setup() {
  ...
  if (!rxQueue) {
    Serial.println("RX queue create FAILED");
    showOnOLED("Init Error", "Queue failed");
  }

  xTaskCreatePinnedToCore(xbeeRxTask, "XBEE_RX", 4096, NULL, 2, NULL, 0);
  xTaskCreatePinnedToCore(apiPostTask, "API_POST", 8192, NULL, 1, NULL, 1);
}


/* ================== LOOP ================== */
void loop() {
  // Work is handled by FreeRTOS tasks.
