...
DROP TABLE IF EXISTS toll_zones CASCADE;
...
-- Insert default toll zone
INSERT INTO toll_zones (name, latitude, longitude) VALUES
('Amrita AB2', 10.903697, 76.899331)
...
CREATE TABLE IF NOT EXISTS esp32_toll_transactions (
    ...
    toll_zone_id INTEGER REFERENCES toll_zones(id) ON DELETE SET NULL,
...
ALTER TABLE esp32_toll_transactions
        ADD COLUMN IF NOT EXISTS toll_zone_id INTEGER REFERENCES toll_zones(id) ON DELETE SET NULL;

UPDATE esp32_toll_transactions AS ett
SET toll_zone_id = tz.id
FROM toll_zones AS tz
WHERE ett.toll_zone_id IS NULL
    AND ROUND(COALESCE(ett.end_lat, 0)::numeric, 6) = ROUND(tz.latitude::numeric, 6)
    AND ROUND(COALESCE(ett.end_lon, 0)::numeric, 6) = ROUND(tz.longitude::numeric, 6);
...
CREATE INDEX IF NOT EXISTS idx_esp32_toll_toll_zone_id ON esp32_toll_transactions(toll_zone_id);
CREATE INDEX IF NOT EXISTS idx_esp32_toll_processed_at ON esp32_toll_transactions(processed_at);
CREATE INDEX IF NOT EXISTS idx_toll_zones_coordinates ON toll_zones(latitude, longitude);
...
BEGIN
    ...
    WHERE vt.type_name = v_vehicle_type;
        ...
        -- Match the reported end coordinates to a configured toll zone when possible
        SELECT tz.id
        INTO v_toll_zone_id
        FROM toll_zones tz
        WHERE tz.is_active = TRUE
            AND ABS(tz.latitude - p_end_lat) <= 0.0005
            AND ABS(tz.longitude - p_end_lon) <= 0.0005
        ORDER BY ABS(tz.latitude - p_end_lat) + ABS(tz.longitude - p_end_lon)
        LIMIT 1;
    ...
    IF v_current_balance < v_calculated_toll THEN
        ...
        INSERT INTO esp32_toll_transactions (
            device_id, vehicle_id, user_id, start_lat, start_lon, end_lat, end_lon, toll_zone_id,
            total_distance_km, toll_amount, wallet_balance_before, 
            wallet_balance_after, status, device_timestamp
        ) VALUES (
            p_device_id, v_vehicle_id, v_user_id, p_start_lat, p_start_lon, p_end_lat, p_end_lon, v_toll_zone_id,
...
DECLARE
    ...
    vehicle1_id INTEGER;
    vehicle2_id INTEGER;
    default_toll_zone_id INTEGER;
BEGIN
    -- Get user ID
    SELECT id INTO test_user_id FROM users WHERE email = 'test@smarttoll.com';
    SELECT id INTO default_toll_zone_id FROM toll_zones WHERE name = 'Amrita AB2';
    ...
    ('ESP32_CAR_001', vehicle1_id, test_user_id, 13.0827, 80.2707, 10.903697, 76.899331, default_toll_zone_id, 15.5, 31.00, 2500.00, 2469.00, 'success', 
     CURRENT_TIMESTAMP - INTERVAL '5 days', CURRENT_TIMESTAMP - INTERVAL '5 days'),
    ('ESP32_CAR_001', vehicle1_id, test_user_id, 12.9716, 77.5946, 10.903697, 76.899331, default_toll_zone_id, 25.2, 50.40, 2469.00, 2418.60, 'success', 
     CURRENT_TIMESTAMP - INTERVAL '3 days', CURRENT_TIMESTAMP - INTERVAL '3 days'),
    ('ESP32_CAR_001', vehicle1_id, test_user_id, 19.0760, 72.8777, 10.903697, 76.899331, default_toll_zone_id, 8.7, 17.40, 2418.60, 2401.20, 'success', 
     CURRENT_TIMESTAMP - INTERVAL '2 days', CURRENT_TIMESTAMP - INTERVAL '2 days'),
    -- Successful bike transactions
    ('ESP32_BIKE_002', vehicle2_id, test_user_id, 28.7041, 77.1025, 10.903697, 76.899331, default_toll_zone_id, 12.3, 12.30, 2401.20, 2388.90, 'success', 
     CURRENT_TIMESTAMP - INTERVAL '1 day', CURRENT_TIMESTAMP - INTERVAL '1 day'),
    ('ESP32_BIKE_002', vehicle2_id, test_user_id, 22.5726, 88.3639, 10.903697, 76.899331, default_toll_zone_id, 6.8, 6.80, 2388.90, 2382.10, 'success', 
     CURRENT_TIMESTAMP - INTERVAL '12 hours', CURRENT_TIMESTAMP - INTERVAL '12 hours'),
    -- Recent transactions
    ('ESP32_CAR_001', vehicle1_id, test_user_id, 11.0168, 76.9558, 10.903697, 76.899331, default_toll_zone_id, 18.4, 36.80, 2382.10, 2345.30, 'success', 
     CURRENT_TIMESTAMP - INTERVAL '6 hours', CURRENT_TIMESTAMP - INTERVAL '6 hours'),
    ('ESP32_BIKE_002', vehicle2_id, test_user_id, 26.9124, 75.7873, 10.903697, 76.899331, default_toll_zone_id, 9.2, 9.20, 2345.30, 2336.10, 'success', 
     CURRENT_TIMESTAMP - INTERVAL '2 hours', CURRENT_TIMESTAMP - INTERVAL '2 hours'),
    -- One failed transaction due to insufficient balance
    ('ESP32_CAR_001', vehicle1_id, test_user_id, 17.3850, 78.4867, 10.903697, 76.899331, default_toll_zone_id, 45.0, 90.00, 50.00, 50.00, 'insufficient_balance', 
