...
function Dashboard() {
  ...
  const [recentTransactions, setRecentTransactions] = useState<TollTransaction[]>([]);
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      const [balanceResponse, vehiclesResponse, transactionsResponse] = await Promise.all([
        fetch(API_ENDPOINTS.wallet.balance, {
          headers: { Authorization: `Bearer ${token}` },
        }),
        fetch(API_ENDPOINTS.vehicles.list, {
          headers: { Authorization: `Bearer ${token}` },
        }),
        fetch(`${API_ENDPOINTS.esp32.tollTransactions}?limit=5`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);
      ...
      <div className="grid gap-4 md:gap-6 lg:gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="border hover:border-gray-400 transition-colors">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3 md:pb-4">
            ...
            <span className="text-lg md:text-xl font-bold text-gray-500">₹</span>
          </CardHeader>
          <CardContent className="pt-0">
            ...
            <p className="text-xs md:text-sm text-gray-500 mb-4 md:mb-6">Available balance</p>
            <Button
        ...
        <Card className="border hover:border-gray-400 transition-colors">
          ...
          <CardContent className="pt-0">
            ...
            <p className="text-xs md:text-sm text-gray-500">Total vehicles</p>
          </CardContent>
        </Card>
        ...
        <Card className="border hover:border-gray-400 transition-colors">
          ...
          <CardContent className="pt-0">
            ...
            <p className="text-xs md:text-sm text-gray-500">Verified account</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-4 md:gap-6 lg:gap-8 grid-cols-1 lg:grid-cols-2">
            ...
            {recentTransactions.length > 0 ? (
              <div className="space-y-4 md:space-y-6">
                {recentTransactions.map((t, i) => (
                  <div key={i} className="flex items-center justify-between py-3 md:py-4 border-b border-gray-200 last:border-0">
                    <div className="flex items-center space-x-3 md:space-x-4">
                      <div className="w-8 h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 bg-gray-100 rounded-lg flex items-center justify-center">
                        <svg className="w-4 h-4 md:w-5 md:h-5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
                        </svg>
                      </div>
                      <div>
                        <p className="text-sm md:text-base font-medium">{t.distance_km}km journey</p>
                        <p className="text-xs md:text-sm text-gray-500 mt-0.5 md:mt-1">
                          {formatDate(t.timestamp || t.device_timestamp || t.created_at || '')}
                        </p>
                      </div>
                    </div>
                    <span className="text-sm md:text-base font-medium">
                      -{formatCurrency(t.toll_amount)}
                    </span>
                  </div>
                ))}
              </div>
              ...
              <div className="text-center py-8 md:py-12">
                <svg className="w-12 h-12 md:w-16 md:h-16 text-gray-400 mx-auto mb-4 md:mb-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
                <p className="text-base md:text-lg text-gray-500 mb-1 md:mb-2">No ESP32 transactions yet</p>
        ...
        <Card className="border hover:border-gray-400 transition-colors">
          ...
          <CardContent className="space-y-3 md:space-y-4 pt-0">
            <Button variant="outline" className="w-full justify-start h-10 md:h-12 text-sm md:text-base" onClick={() => router.push('/vehicles')}>
              <svg className="mr-2 md:mr-3 h-4 w-4 md:h-5 md:w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
              </svg>
              Manage Vehicles
            </Button>
            <Button variant="outline" className="w-full justify-start h-10 md:h-12 text-sm md:text-base" onClick={() => router.push('/history')}>
              <svg className="mr-2 md:mr-3 h-4 w-4 md:h-5 md:w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              View Transaction History
            </Button>
            <Button variant="outline" className="w-full justify-start h-10 md:h-12 text-sm md:text-base" onClick={() => router.push('/recharge')}>
