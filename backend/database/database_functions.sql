-- CORRECTED DATABASE SCHEMA FOR SMART TOLL SYSTEM
-- This schema aligns with the implemented models and GPS-based toll system
...
CREATE TABLE vehicles (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID REFERENCES users(id) ON DELETE CASCADE,
    license_plate TEXT UNIQUE NOT NULL,  -- Changed from plate_number to match models
...
-- TOLL ROAD ZONES (PostGIS polygon-based zones with integrated pricing)
CREATE TABLE toll_road_zones (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    name TEXT NOT NULL,
    description TEXT,
...
-- VEHICLE TOLL HISTORY (GPS-based entry/exit tracking)
CREATE TABLE vehicle_toll_history (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- TRANSACTIONS (Enhanced with comprehensive tracking)
...
-- GPS LOGS (PostGIS point storage with separate lat/lon for compatibility)
...
-- RECHARGES (Razorpay payment tracking - if needed separately)
...
-- PostGIS functions for Smart Toll System
...
AS $$
BEGIN
    -- Use PostGIS to calculate distance in meters and convert to kilometers
    RETURN ST_Distance(
...
BEGIN
    -- Get all GPS points for the vehicle in the time range, ordered by time
    FOR curr_point IN
        SELECT location
        FROM gps_logs
    ...
    LOOP
        ...
        IF prev_point IS NOT NULL THEN
            ...
            -- Only add reasonable distances (ignore GPS jumps)
            IF point_distance < 5.0 THEN -- Max 5km between consecutive points
...
-- Function to get vehicle GPS statistics
CREATE OR REPLACE FUNCTION get_vehicle_gps_stats(
    p_vehicle_id UUID,
...
BEGIN
    ...
    SELECT 
        ...
        calculate_vehicle_distance(p_vehicle_id, p_start_date, p_end_date) as total_distance_km,
        ROUND(AVG(g.speed), 2) as average_speed,
        MAX(g.speed) as max_speed,
        MIN(g.logged_at) as first_log,
        MAX(g.logged_at) as last_log
    FROM gps_logs g
    WHERE g.vehicle_id = p_vehicle_id
    AND g.logged_at BETWEEN p_start_date AND p_end_date
    AND g.speed IS NOT NULL;
END;
$$;

-- Function to get GPS data for heat map
CREATE OR REPLACE FUNCTION get_gps_heatmap_data(
    p_north NUMERIC,
    p_south NUMERIC,
    p_east NUMERIC,
    p_west NUMERIC,
...
BEGIN
    RETURN QUERY
    SELECT 
        ROUND(g.latitude, 4) as latitude,
        ROUND(g.longitude, 4) as longitude,
        COUNT(*)::BIGINT as intensity
    FROM gps_logs g
    WHERE g.logged_at BETWEEN p_start_date AND p_end_date
    AND ST_Within(
        g.location,
        ST_MakeEnvelope(p_west, p_south, p_east, p_north, 4326)
    )
    GROUP BY ROUND(g.latitude, 4), ROUND(g.longitude, 4)
    HAVING COUNT(*) >= 5  -- Only include points with at least 5 logs
...
-- Function to get toll zones within geographical bounds
...
BEGIN
    ...
    SELECT 
        ...
        tz.rate_per_km,
        tz.is_active,
        ROUND(ST_Area(tz.zone_polygon::geography) / 1000000.0, 2) as zone_area_sq_km
    FROM toll_road_zones tz
    WHERE tz.is_active = true
    AND ST_Intersects(
        tz.zone_polygon,
...
-- Function to get toll zone performance analytics
...
CREATE TRIGGER update_toll_zones_updated_at BEFORE UPDATE ON toll_road_zones 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
